...
func Post_alerting_rule_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePost_alerting_rule_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_api_alerting_rule_id",
		mcp.WithDescription("Create a rule"),
		mcp.WithString("kbn-xsrf", mcp.Required(), mcp.Description("A required header to protect against CSRF attacks")),
		mcp.WithString("id", mcp.Description("The identifier for the rule. If it is omitted, an ID is randomly generated.")),
		...
		mcp.WithObject("schedule", mcp.Required(), mcp.Description("Input parameter: The check interval, which specifies how frequently the rule conditions are checked.")),
		...
		mcp.WithString("consumer", mcp.Required(), mcp.Description("Input parameter: The name of the application or feature that owns the rule. For example: `alerts`, `apm`, `discover`, `infrastructure`, `logs`, `metrics`, `ml`, `monitoring`, `securitySolution`, `siem`, `stackAlerts`, or `uptime`.")),
		mcp.WithBoolean("enabled", mcp.Description("Input parameter: Indicates whether you want to run the rule on an interval basis after it is created.")),
		...
		mcp.WithString("rule_type_id", mcp.Required(), mcp.Description("Input parameter: The rule type identifier.")),
		mcp.WithArray("tags", mcp.Description("Input parameter: The tags for the rule.")),
		mcp.WithArray("actions", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_alerting_rule_idHandler(cfg),
	}
}
