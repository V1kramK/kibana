...
func Get_fleet_agent_policiesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["noAgentCount"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("noAgentCount=%v", val))
		}
		if val, ok := args["withAgentCount"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("withAgentCount=%v", val))
		}
...
func CreateGet_fleet_agent_policiesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_fleet_agent_policies",
		mcp.WithDescription("Get agent policies"),
		...
		mcp.WithBoolean("noAgentCount", mcp.Description("use withAgentCount instead")),
		mcp.WithBoolean("withAgentCount", mcp.Description("get policies with agent count")),
		mcp.WithBoolean("full", mcp.Description("get full policies with package policies populated")),
		mcp.WithString("format", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_fleet_agent_policiesHandler(cfg),
	}
}
