...
func Get_streams_nameHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_streams_nameTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_streams_name",
		mcp.WithDescription("Get a stream"),
		mcp.WithString("name", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_streams_nameHandler(cfg),
	}
}
