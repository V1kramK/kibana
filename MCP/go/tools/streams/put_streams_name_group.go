...
func Put_streams_name_groupHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/streams/%s/_group", cfg.BaseURL, name)
...
func CreatePut_streams_name_groupTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_api_streams_name__group",
		mcp.WithDescription("Upsert group stream settings"),
		...
		mcp.WithObject("group", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Put_streams_name_groupHandler(cfg),
	}
}
