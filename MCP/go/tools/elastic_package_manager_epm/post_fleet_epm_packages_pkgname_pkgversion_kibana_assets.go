...
func Post_fleet_epm_packages_pkgname_pkgversion_kibana_assetsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/fleet/epm/packages/%s/%s/kibana_assets", cfg.BaseURL, pkgName, pkgVersion)
...
func CreatePost_fleet_epm_packages_pkgname_pkgversion_kibana_assetsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_api_fleet_epm_packages_pkgName_pkgVersion_kibana_assets",
		mcp.WithDescription("Install Kibana assets for a package"),
		...
		mcp.WithArray("space_ids", mcp.Description("Input parameter: When provided install assets in the specified spaces instead of the current space.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_fleet_epm_packages_pkgname_pkgversion_kibana_assetsHandler(cfg),
	}
}
