...
func Delete_fleet_epm_packages_pkgname_pkgversion_datastream_assetsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["packagePolicyId"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("packagePolicyId=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/api/fleet/epm/packages/%s/%s/datastream_assets%s", cfg.BaseURL, pkgName, pkgVersion, queryString)
...
func CreateDelete_fleet_epm_packages_pkgname_pkgversion_datastream_assetsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_fleet_epm_packages_pkgName_pkgVersion_datastream_assets",
		mcp.WithDescription("Delete assets for an input package"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Delete_fleet_epm_packages_pkgname_pkgversion_datastream_assetsHandler(cfg),
	}
}
