...
func Put_fleet_epm_packages_pkgname_pkgversionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/fleet/epm/packages/%s/%s", cfg.BaseURL, pkgName, pkgVersion)
...
func CreatePut_fleet_epm_packages_pkgname_pkgversionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_api_fleet_epm_packages_pkgName_pkgVersion",
		mcp.WithDescription("Update package settings"),
		...
		mcp.WithBoolean("keepPoliciesUpToDate", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Put_fleet_epm_packages_pkgname_pkgversionHandler(cfg),
	}
}
