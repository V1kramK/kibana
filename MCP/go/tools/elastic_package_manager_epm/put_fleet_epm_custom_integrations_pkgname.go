...
func Put_fleet_epm_custom_integrations_pkgnameHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/fleet/epm/custom_integrations/%s", cfg.BaseURL, pkgName)
...
func CreatePut_fleet_epm_custom_integrations_pkgnameTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_api_fleet_epm_custom_integrations_pkgName",
		mcp.WithDescription("Update a custom integration"),
		...
		mcp.WithString("readMeData", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Put_fleet_epm_custom_integrations_pkgnameHandler(cfg),
	}
}
