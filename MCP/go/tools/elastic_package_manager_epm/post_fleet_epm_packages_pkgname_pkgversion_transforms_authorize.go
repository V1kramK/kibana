...
func Post_fleet_epm_packages_pkgname_pkgversion_transforms_authorizeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/fleet/epm/packages/%s/%s/transforms/authorize%s", cfg.BaseURL, pkgName, pkgVersion, queryString)
...
func CreatePost_fleet_epm_packages_pkgname_pkgversion_transforms_authorizeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_api_fleet_epm_packages_pkgName_pkgVersion_transforms_authorize",
		mcp.WithDescription("Authorize transforms"),
		...
		mcp.WithArray("transforms", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_fleet_epm_packages_pkgname_pkgversion_transforms_authorizeHandler(cfg),
	}
}
