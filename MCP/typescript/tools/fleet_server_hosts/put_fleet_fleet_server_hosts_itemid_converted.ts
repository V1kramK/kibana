/**
 * MCP Server function for Update a Fleet Server host
 */
...
function getPut_Api_Fleet_Fleet_Server_Hosts_Item_IdHandler(config: APIConfig): (ctx: any, request: MCPRequest) => Promise<MCPToolResult> {
    ...
    if (args.host_urls !== undefined) {
        queryParams.push(`host_urls=${args.host_urls}`);
            ...
            const url = `${config.baseUrl}/api/v2/put_api_fleet_fleet_server_hosts_item_id${queryString}`;
...
function createPut_Api_Fleet_Fleet_Server_Hosts_Item_IdTool(config: APIConfig): Tool {
    return {
        definition: {
            name: "put_api_fleet_fleet_server_hosts_item_id",
        ...
        proxy_id: { type: "string", required: true, description: "" },
        ...
        host_urls: { type: "string", required: false, description: "" },
            }
        },
        handler: getPut_Api_Fleet_Fleet_Server_Hosts_Item_IdHandler(config)
    };
}
...
export {
    ...
    Tool,
    ToolDefinition,
    getPut_Api_Fleet_Fleet_Server_Hosts_Item_IdHandler,
    createPut_Api_Fleet_Fleet_Server_Hosts_Item_IdTool
};