/**
 * MCP Server function for Get package policies
 */
...
function getGet_Api_Fleet_Package_PoliciesHandler(config: APIConfig): (ctx: any, request: MCPRequest) => Promise<MCPToolResult> {
    ...
    if (args.withAgentCount !== undefined) {
        queryParams.push(`withAgentCount=${args.withAgentCount}`);
            ...
            const url = `${config.baseUrl}/api/v2/get_api_fleet_package_policies${queryString}`;
...
function createGet_Api_Fleet_Package_PoliciesTool(config: APIConfig): Tool {
    return {
        definition: {
            name: "get_api_fleet_package_policies",
        ...
        withAgentCount: { type: "string", required: false, description: "" },
            }
        },
        handler: getGet_Api_Fleet_Package_PoliciesHandler(config)
    };
}
...
export {
    ...
    Tool,
    ToolDefinition,
    getGet_Api_Fleet_Package_PoliciesHandler,
    createGet_Api_Fleet_Package_PoliciesTool
};