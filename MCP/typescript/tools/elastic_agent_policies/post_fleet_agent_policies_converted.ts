/**
 * MCP Server function for Create an agent policy
 */
...
function getPost_Api_Fleet_Agent_PoliciesHandler(config: APIConfig): (ctx: any, request: MCPRequest) => Promise<MCPToolResult> {
    ...
    if (args.sys_monitoring !== undefined) {
        queryParams.push(`sys_monitoring=${args.sys_monitoring}`);
    }
    ...
    if (args.monitoring_enabled !== undefined) {
        queryParams.push(`monitoring_enabled=${args.monitoring_enabled}`);
            ...
            const url = `${config.baseUrl}/api/v2/post_api_fleet_agent_policies${queryString}`;
...
function createPost_Api_Fleet_Agent_PoliciesTool(config: APIConfig): Tool {
    return {
        definition: {
            name: "post_api_fleet_agent_policies",
        ...
        sys_monitoring: { type: "string", required: false, description: "" },
        ...
        handler: getPost_Api_Fleet_Agent_PoliciesHandler(config)
    };
}
...
export {
    ...
    Tool,
    ToolDefinition,
    getPost_Api_Fleet_Agent_PoliciesHandler,
    createPost_Api_Fleet_Agent_PoliciesTool
};