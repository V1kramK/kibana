/**
 * MCP Server function for Create an agent binary download source
 */
...
function getPost_Api_Fleet_Agent_Download_SourcesHandler(config: APIConfig): (ctx: any, request: MCPRequest) => Promise<MCPToolResult> {
    ...
    if (args.ssl !== undefined) {
        queryParams.push(`ssl=${args.ssl}`);
            ...
            const url = `${config.baseUrl}/api/v2/post_api_fleet_agent_download_sources${queryString}`;
...
function createPost_Api_Fleet_Agent_Download_SourcesTool(config: APIConfig): Tool {
    return {
        definition: {
            name: "post_api_fleet_agent_download_sources",
        ...
        handler: getPost_Api_Fleet_Agent_Download_SourcesHandler(config)
    };
}
...
export {
    ...
    Tool,
    ToolDefinition,
    getPost_Api_Fleet_Agent_Download_SourcesHandler,
    createPost_Api_Fleet_Agent_Download_SourcesTool
};