/**
 * MCP Server function for Bulk reassign agents
 */
...
function getPost_Api_Fleet_Agents_Bulk_ReassignHandler(config: APIConfig): (ctx: any, request: MCPRequest) => Promise<MCPToolResult> {
    ...
    if (args.includeInactive !== undefined) {
        queryParams.push(`includeInactive=${args.includeInactive}`);
            ...
            const url = `${config.baseUrl}/api/v2/post_api_fleet_agents_bulk_reassign${queryString}`;
...
function createPost_Api_Fleet_Agents_Bulk_ReassignTool(config: APIConfig): Tool {
    return {
        definition: {
            name: "post_api_fleet_agents_bulk_reassign",
        ...
        handler: getPost_Api_Fleet_Agents_Bulk_ReassignHandler(config)
    };
}
...
export {
    ...
    Tool,
    ToolDefinition,
    getPost_Api_Fleet_Agents_Bulk_ReassignHandler,
    createPost_Api_Fleet_Agents_Bulk_ReassignTool
};